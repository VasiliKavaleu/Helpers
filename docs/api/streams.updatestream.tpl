<doc:page:ru toc="Aipix/Media server/Web API/Streams/UpdateStream" title="Streams.UpdateStream">
    <page:p>Изменение конфигурации и параметров ранее созданного потока.</page:p>
    <page:info>Метод не может изменить UID потока, первичную ноду кластера и dvr chunk.</page:info>
    <api:method>
        <method:request type="PUT" route="/streams/{stream-uid}/"/>
        ...
        <method:body type="json" desc="Объект с информацией о интервале получения событий">
            ...
            <method:bodyparam code="200" name = "location" type="string" desc="Информационный параметр место расположения источника"/>
        </method:body>
        <method:return code="200" type="json" desc="Поток успешно обновлен и будет перезапущен"/>
